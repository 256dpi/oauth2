...
func ProcessTokenRequest(d Delegate, r *http.Request) (*oauth2.TokenRequest, Client, Error) {
	...
	if !oauth2.KnownGrantType(req.GrantType) {
		return nil, nil, &OAuth2Error{
			Error: oauth2.InvalidRequest(oauth2.NoState, "Unknown grant type"),
		}
	}
	...
	if err == ErrNotFound {
		return nil, nil, &OAuth2Error{
			Error: oauth2.InvalidClient(oauth2.NoState, "Unknown client"),
		}
	} else if err != nil {
		return nil, nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to lookup client"),
		}
	}
...
func HandlePasswordGrant(d Delegate, c Client, r *oauth2.TokenRequest) (*oauth2.TokenResponse, Error) {
	...
	if err == ErrNotFound {
		return nil, &OAuth2Error{
			Error: oauth2.AccessDenied(oauth2.NoState, "Unknown resource owner"),
		}
	} else if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to lookup resource owner"),
		}
	}
	...
	if err == ErrRejected {
		...
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to grant scope"),
		}
	}
...
func HandleAuthorizationCodeGrant(d AuthorizationCodeDelegate, c Client, r *oauth2.TokenRequest) (*oauth2.TokenResponse, Error) {
	...
	if err == ErrMalformed {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidRequest(oauth2.NoState, "Malformed authorization code"),
		}
	} else if err == ErrNotFound {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Unknown authorization code"),
		}
	} else if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to lookup authorization code"),
		}
	}
	...
	if ac.ExpiresAt().Before(time.Now()) {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Expired authorization code"),
		}
	}
	...
	if ac.RedirectURI() != r.RedirectURI {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Changed redirect uri"),
		}
	}
	...
	if ac.ClientID() != c.ID() {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Invalid authorization code ownership"),
		}
	}
	...
	if ac.ResourceOwnerID() != "" {
		...
		if err == ErrNotFound {
			return nil, &OAuth2Error{
				Source: err,
				Error:  oauth2.ServerError(oauth2.NoState, "Expected to find resource owner"),
			}
		} else if err != nil {
			return nil, &OAuth2Error{
				Source: err,
	...
	if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to remove authorization code"),
		}
	}
...
func HandleRefreshTokenGrant(d RefreshTokenDelegate, c Client, r *oauth2.TokenRequest) (*oauth2.TokenResponse, Error) {
	...
	if err == ErrMalformed {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidRequest(oauth2.NoState, "Malformed refresh token"),
		}
	} else if err == ErrNotFound {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Unknown refresh token"),
		}
	} else if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to lookup refresh token"),
		}
	}
	...
	if rt.ExpiresAt().Before(time.Now()) {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Expired refresh token"),
		}
	}
	...
	if !rt.Scope().Includes(r.Scope) {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidScope(oauth2.NoState, "New scope exceeds granted scope"),
		}
	}
	...
	if rt.ClientID() != c.ID() {
		return nil, &OAuth2Error{
			Error: oauth2.InvalidGrant(oauth2.NoState, "Invalid refresh token ownership"),
		}
	}
	...
	if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to remove refresh token"),
		}
	}
...
func BuildTokenResponse(d Delegate, c Client, ro ResourceOwner, scope oauth2.Scope) (*oauth2.TokenResponse, Error) {
	...
	if err != nil {
		return nil, &OAuth2Error{
			Source: err,
			Error:  oauth2.ServerError(oauth2.NoState, "Failed to issue access token"),
		}
	}
	...
	if ok {
		...
		if err != nil {
			return nil, &OAuth2Error{
				Source: err,
				Error:  oauth2.ServerError(oauth2.NoState, "Failed to issue refresh token"),
			}
		}
